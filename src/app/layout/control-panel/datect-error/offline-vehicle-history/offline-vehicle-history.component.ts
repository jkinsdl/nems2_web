...
export class OfflineVehicleHistoryComponent implements OnInit {
  ...
  @ViewChild('offlineVehicleHistoryGrid', { read: ElementRef }) offlineVehicleHistoryGrid : ElementRef;
  ...
  onBtExport() {
    this.gbpacketService.getGbpacketInvalid(this.searchFilter).subscribe(res=>{
      console.log(res)
      this.utilService.gridDataToExcelData("offline vehicle history", this.gridApi ,res.body)
    },error=>{
      console.log(error)
    })
  }
